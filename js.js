...
function imgclick() {
	...
	setTimeout(function(){thimgclick.style.width = "70%";} , 1000);
	setTimeout(function(){thimgclick.style.margin = "10%";} , 1000);
	...
	setTimeout(function(){document.getElementById("popupactive").click();} , 2100);
	setTimeout(function(){document.getElementById('popupsideimg').appendChild(thimgclick);} , 2100);
}
//imgclick
//Ajax request
...
function rgetgift(getgiftemail) {
	...
	if (document.getElementById('popupemail') == null) {
		...
		para.innerHTML += '<input required id="popupemail" type="email" placeholder="Put Your Email to claim your Gift">';
	}else {
         xmlhttp.onreadystatechange = function() {
             if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
             	para.innerHTML = xmlhttp.responseText;
         ...
         var rgetgiftemail = (document.getElementById('popupemail') != null) ? document.getElementById('popupemail').value : '';
...
function animations() {
	...
	var arr2 = ['pulse', 'shake','wobble','jello', 'flip' ,'rubberBand','swing', 'hinge'];
	rand = Math.floor(Math.random() * (arr2.length));
	thanimations.className = "animated "+arr2[rand];
	setTimeout(function(){thanimations.className = "";}, 1000)

}

window.onload = function(){
	var nicepopup = document.getElementById('nicepopup');
	nicepopup.style.left = "calc((100vw - "+nicepopup.offsetWidth+"px) / 2)";
	// setTimeout(function(){
